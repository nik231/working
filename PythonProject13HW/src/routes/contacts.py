...
@router.get('/', response_model=list[ContactResponseSchema], dependencies=[Depends(RateLimiter(times=2, seconds=60))])
...
async def get_contact_by_surname(contact_surname: str, db: AsyncSession = Depends(get_db),
    ...
    if contact is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"Contact with surname {contact_surname} not found")
    return contact
...
@router.post('/', response_model=ContactResponseSchema, status_code=status.HTTP_201_CREATED,dependencies=[Depends(RateLimiter(times=1, seconds=60))])
...
@router.put('/{contact_id}', response_model=ContactUpdateSchema, dependencies=[Depends(RateLimiter(times=2, seconds=60))])
